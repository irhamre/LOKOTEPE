...
// TX QUEUE
// ============================================================
#define TX_QUEUE_SIZE  96   // 30 sensor × 3 buffer
...
void printRadioDiag() {
    ...
    float txRate = diag.txCount > 0 ? (diag.txOk * 100.0f / diag.txCount) : 0.0f;
    int qSize = (txHead - txTail + TX_QUEUE_SIZE) % TX_QUEUE_SIZE;
    ...
    Serial.printf( "║ Queue now    : %d\n", qSize);
    Serial.println("╠══════════════════════════════════════╣");
    for (int i = 0; i < 30; i++)
        Serial.printf("║   Tire %-2d    : %lu paket TX\n", i, diag.txPerTire[i]);
    Serial.println("╚══════════════════════════════════════╝");
    Serial.println();
}

bool txQueuePush(const TpmsData& d) {
    // Update data jika tire sudah ada di queue
    int pos = txTail;
    while (pos != txHead) {
        if (txQueue[pos].tireId == d.tireId) {
            txQueue[pos] = d;
            return true;
        }
...
const char* tireLabels[TIRE_COUNT] = {
    ...
    "Mid Left",      "Mid Left-2",    "Mid Left-3",
    "Mid Right",     "Mid Right-2",   "Mid Right-3",
    "Rear-2 Left",   "Rear-2 Left-2", "Rear-2 Left-3",
    "Rear-2 Right",  "Rear-2 Right-2","Rear-2 Right-3",
...
void printValidPositions() {
    ...
    Serial.println("  ml/ml-2/ml-3   mr/mr-2/mr-3");
    Serial.println("  r2l/r2l-2/r2l-3   r2r/r2r-2/r2r-3");
...
void printAllTires() {
    ...
    Serial.println();
    Serial.println("==========================================================================");
    Serial.printf( "TPMS REPORT | Uptime %02lu:%02lu:%02lu | 30 Sensor\n",
...
class MyAdvertisedDeviceCallbacks : public NimBLEAdvertisedDeviceCallbacks {
    void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
        ...
        if (!txQueuePush(pkt)) {
            diag.queueDropped++;
            Serial.printf("[WARN] Queue penuh, ID:%d dibuang\n", idx);
        }

        unsigned long now_ms = millis();
        char codeStr[8];
        snprintf(codeStr, sizeof(codeStr), "0x%04X", statusCode);
        Serial.printf("[BLE] %-13s | %s | %s | %.1fkPa | %dC | %.2fV | RSSI:%d",
                      tireLabels[idx], codeStr, tireData[idx].statusStr.c_str(),
                      kpaGauge, celsius, voltage, advertisedDevice->getRSSI());
        if (lastSeenBLE[idx] > 0)
            Serial.printf(" | %.1fs\n", (now_ms - lastSeenBLE[idx]) / 1000.0f);
        else
...
void handleSerial() {
    ...
    if (input == "diag") { printRadioDiag(); return; }
    ...
    if (input.startsWith("p-")) {
        ...
        if (spaceIdx == -1) { Serial.println("[ERROR] Format: p-fl <MAC>"); return; }
        String pos = input.substring(2, spaceIdx);
        String mac = input.substring(spaceIdx + 1);
        mac.trim(); mac.toLowerCase();
        if (mac.length() != 17) { Serial.println("[ERROR] Format MAC salah."); return; }
        int idx = getIndexFromCommand(pos);
        if (idx == -1) { Serial.println("[ERROR] Posisi tidak valid."); printValidPositions(); return; }
        ...
        lastSeenBLE[idx] = 0;
        logLine("[OK] " + String(tireLabels[idx]) + " → " + mac);
        return;
    }
    ...
    if (input.startsWith("unpair-")) {
        ...
        if (pairedMacs[idx].empty()) { Serial.printf("[ERROR] %s belum dipasangkan.\n", tireLabels[idx]); return; }
        pairedMacs[idx] = "";
        saveMacToFlash(idx, "");
        tireData[idx]    = TireData{};
        lastSeenBLE[idx] = 0;
        logLine("[OK] " + String(tireLabels[idx]) + " di-unpair.");
        return;
    }
    ...
    Serial.println("Perintah: scan_tpms | p-<pos> <MAC> | unpair-<pos> | unpair_all | status | report | diag | diag_reset");
    printValidPositions();
}
...
void setup() {
    Serial.begin(115200);
    delay(500);
    Serial.println("\n=== TPMS BLE + CC1101 GATEWAY (30 SENSOR) ===");
    ...
    Serial.println("[BLE] Scanner aktif. 30 sensor siap dipair.");
}
...
void loop() {
    ...
    if (scanTpmsDone) {
        ...
        if (scannedTpms.empty()) {
            ...
            Serial.println("[TIP] Gunakan: p-<pos> <MAC> untuk pair sensor");
        }
        NimBLEDevice::getScan()->clearResults();
        NimBLEDevice::getScan()->start(0, nullptr, false);
    }
